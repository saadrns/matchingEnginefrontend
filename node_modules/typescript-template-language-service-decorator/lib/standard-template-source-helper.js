...
var StandardTemplateContext = /** @class */ (function () {
    function StandardTemplateContext(fileName, node, helper, templateStringSettings) {
    ...
    Object.defineProperty(StandardTemplateContext.prototype, "text", {
        get: function () {
            var literalContents = this.node.getText().slice(1, -1);
            if (this.node.kind === ts.SyntaxKind.NoSubstitutionTemplateLiteral) {
                return literalContents;
            }
            var stringStart = this.node.getStart() + 1;
            var contents = literalContents;
            var nodeStart = this.node.head.end - stringStart - 2;
            for (var _i = 0, _a = this.node.templateSpans.map(function (x) { return x.literal; }); _i < _a.length; _i++) {
                var child = _a[_i];
    ...
    StandardTemplateContext.prototype.getSubstitution = function (templateString, start, end) {
...
var StandardTemplateSourceHelper = /** @class */ (function () {
    function StandardTemplateSourceHelper(templateStringSettings, helper) {
        this.templateStringSettings = templateStringSettings;
        this.helper = helper;
    }
    StandardTemplateSourceHelper.prototype.getTemplate = function (fileName, position) {
        ...
        return new StandardTemplateContext(fileName, node, this.helper, this.templateStringSettings);
    };
    ...
    StandardTemplateSourceHelper.prototype.getValidTemplateNode = function (templateStringSettings, node) {
        ...
        switch (node.kind) {
            ...
            case ts.SyntaxKind.NoSubstitutionTemplateLiteral:
                if (nodes_1.isTaggedLiteral(node, templateStringSettings.tags)) {
                    return node;
                }
                return undefined;
